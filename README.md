...
> 默认使用 CUDA（需 NVIDIA GPU）。无 GPU 时，修改 `edge/training/config.py` 中 `DEVICE = "cuda"` 改为 `"cpu"` 或 `"mps"`。
...
| 优先级 | 组件 | 说明 |
|--------|------|------|
| 1 | PostgreSQL + MQTT Broker | 基础设施，**必须先启动** |
| 2 | Cloud API (`python -m cloud.main`) | 依赖数据库，**Agent 复核需配置 LLM** |
| 3 | Edge Server (`python -m edge.main --server`) | 依赖云端 API |
| 4 | Web 前端 (`npm run dev`) | 仅展示层，最后启动 |

> 使用一键脚本 `start.ps1` / `start.sh` 可自动按顺序启动。

### 基础设施（PostgreSQL + MQTT）
...
前端正对 Agent 复核结果做了明确的状态区分：
- ✅ **复核完成**（绿色）— Agent 正常返回结论
- ⚠ **未配置**（橙色）— LLM API Key 未设置，提示点击 🤖 配置
- ⚠ **调用失败**（橙色）— 网络或模型异常，显示具体错误信息
- ⚠ **无输出**（橙色）— 模型运行但未返回有效内容
- ⏳ **等待复核**（黄色闪烁）— 后台排队中
...
```
 边云协同检测        🤖 gpt-4o   ●边缘   ●云端   ◉auto-refresh
 ┌──────────────────┬──────────────────────────────────────────┐
 │ [摄像头◉/文件○]   │ 检测记录 & Agent 复核   共12 本地7 复核5 │
 │ [0][置信度 0.3]  │ 更新于 14:35:22                          │
 │ [▶ 开始]         │ ┌────────────────────────────────────────┐│
 │                  │ │ 🖼 14:35 裂纹,夹杂  3处 需复核 72% 复核││
 │   MJPEG 视频流   │ │ ✓ 复核完成                             ││
 │  (YOLO 标注)     │ │ 该裂纹长度超过标准…                     ││
 │                  │ ├────────────────────────────────────────┤│
 │                  │ │ 🖼 14:33 氧化皮   高置信本地   89% 本地││
 │                  │ ├────────────────────────────────────────┤│
 │                  │ │ 🖼 14:32 斑块     低置信   45% 复核 ⚠  ││
 │                  │ │ ⚠ 未配置 API Key                       ││
 │                  │ │ 💡 点击右上角 🤖 图标配置 API Key       ││
...
- **右栏** — 缺陷记录列表（带缩略图、缺陷数量、原因标签），点击展开大图
- **复核记录** — 左侧红色竖线标识，展开后显示 Agent 结论 + 状态徽章
- **模型切换** — 点击右上角 🤖 可实时切换 LLM 模型、API Key、地址
...
| 步骤 | 优先级 | 操作 | 说明 |
|------|--------|------|------|
| 1/5 | P1 | 数据库连通性检测 | 通过 `.env` 配置连接 PostgreSQL，失败则退出 |
| 2/5 | P1 | MQTT Broker 检测 | socket 连通性检查，失败则警告但不中断（回退 HTTP） |
| 3/5 | P2 | 启动 Cloud API | `python -m cloud.main` → 端口 8000，lifespan 中自动连接 MQTT |
| 4/5 | P3 | 启动 Edge Server | `python -m edge.main --server` → 端口 8080，启动时自动连接 MQTT |
| 5/5 | P4 | 启动 Web 前端 | `npm run dev` → 端口 5173 |
